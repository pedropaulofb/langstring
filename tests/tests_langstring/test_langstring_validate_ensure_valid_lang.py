...
def test_validate_ensure_valid_lang_non_standard_codes(lang: str) -> None:
    """
    Test the _validate_ensure_valid_lang method with non-standard language codes when VALID_LANG is enabled.

    :param lang: The language code to test.
    :raises AssertionError: If LangString accepts a non-standard language code when VALID_LANG is enabled.
    """
    Controller.set_flag(LangStringFlag.VALID_LANG, True)
    with pytest.raises(ValueError, match="VALID_LANG enabled: LangString's 'lang' field cannot"):
        LangString("Test", lang)
...
def test_validate_ensure_valid_lang_null_or_undefined(lang) -> None:
    """
    Test the _validate_ensure_valid_lang method with null or undefined language codes when VALID_LANG is enabled.

    :param lang: The language code to test (None or "undefined").
    :raises AssertionError: If LangString accepts null or undefined language codes when VALID_LANG is enabled.
    """
    Controller.set_flag(LangStringFlag.VALID_LANG, True)
    with pytest.raises(ValueError, match="VALID_LANG enabled: LangString's 'lang' field cannot"):
        LangString("Test", lang)

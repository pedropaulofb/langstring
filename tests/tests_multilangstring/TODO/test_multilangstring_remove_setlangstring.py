import pytest
from langstring import MultiLangString, SetLangString

@pytest.mark.parametrize("lang, texts, clean_empty, expected_result", [
    # Default case
...
@pytest.mark.parametrize("initial_contents, lang_to_remove, clean_empty, expected_contents", [
    ...
    ({"en": {"hello", "world"}}, "en", False, {"en": {"world"}}),
    # SetLangString with multiple texts removing multiple entries
    ({"en": {"hello", "world", "test"}}, "en", False, {"en": {"test"}}),
])
...
@pytest.mark.parametrize("initial_contents, invalid_input, clean_empty, expected_exception, expected_message", [
    # Attempting to remove with an invalid type for SetLangString
    ({"en": {"hello"}}, 123, False, TypeError, "Expected SetLangString for removal, received 'int'"),
])
def test_remove_setlangstring_invalid_cases(initial_contents, invalid_input, clean_empty, expected_exception, expected_message):

...
@pytest.mark.parametrize("initial_contents, discarding_contents, expected_result", [
    ...
    ({"en": {"Hello"}, "fr": {"Bonjour"}, "es": {"Hola"}}, {"fr": {"Bonjour"}, "es": {"Hola"}}, {"en": {"Hello"}, "fr": set(), "es": set()}),
])
...
@pytest.mark.parametrize("initial_contents, discarding_contents, expected_result", [
    ({"en": {"Hello", "World"}, "fr": {"Bonjour"}}, {"en": {"Goodbye"}, "fr": {"Salut"}}, {"en": {"Hello", "World"}, "fr": {"Bonjour"}}),
])
def test_discard_multilangstring_overlapping_languages_different_texts(initial_contents, discarding_contents, expected_result):
    mls_initial = MultiLangString(initial_contents)
    mls_discarding = MultiLangString(discarding_contents)
    mls_initial.discard_multilangstring(mls_discarding)
    assert mls_initial.mls_dict == expected_result, "Discarding did not work as expected for overlapping languages with different texts."

@pytest.mark.parametrize("initial_contents, discarding_contents, expected_result", [
    ({"en": {"Hello", "World"}, "fr": {"Bonjour"}, "es": {"Hola"}}, {"fr": {"Bonjour"}}, {"en": {"Hello", "World"}, "fr": set(), "es": {"Hola"}}),
])
...
def test_discard_multilangstring_additional_languages_not_in_original(initial_contents, discarding_contents, expected_result):
    mls_initial = MultiLangString(initial_contents)
    mls_discarding = MultiLangString(discarding_contents)
    mls_initial.discard_multilangstring(mls_discarding)
    assert mls_initial.mls_dict == expected_result, "Discarding did not work as expected when discarding object has additional languages."

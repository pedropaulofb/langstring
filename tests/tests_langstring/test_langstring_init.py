...
def test_langstring_init_with_flags(text, lang, ensure_text, ensure_valid_lang, expected_exception) -> None:
    ...
    :param ensure_text: The state of the DEFINED_TEXT flag.
    :param ensure_valid_lang: The state of the VALID_LANG flag.
    :param expected_exception: The expected exception type, if any.
    """
    Controller.set_flag(LangStringFlag.DEFINED_TEXT, ensure_text)
    Controller.set_flag(LangStringFlag.VALID_LANG, ensure_valid_lang)
...
def test_langstring_with_all_flags_enabled_invalid_language() -> None:
    ...
    with pytest.raises(ValueError, match="VALID_LANG enabled: LangString's 'lang' field cannot"):
...
def test_langstring_with_all_flags_enabled_empty_text() -> None:
    ...
    with pytest.raises(ValueError, match="DEFINED_TEXT enabled: LangString's 'text' field cannot"):
        LangString("", "en")
...
def test_langstring_with_mixed_flags() -> None:
    """Test LangString initialization with DEFINED_TEXT and VALID_LANG enabled, but ENSURE_ANY_LANG disabled."""
    Controller.set_flag(LangStringFlag.DEFINED_TEXT, True)
    Controller.set_flag(LangStringFlag.VALID_LANG, True)

    with pytest.raises(ValueError, match="VALID_LANG enabled: LangString's 'lang'"):
        LangString("Hello", "invalid-lang")
...
def test_invalid_language_with_ensure_valid_lang_disabled() -> None:
    """
    Test the initialization of a LangString object with an invalid language when VALID_LANG is disabled.
    ...
    assert ls.lang == "invalid-lang", "LangString should allow invalid language when VALID_LANG is disabled"

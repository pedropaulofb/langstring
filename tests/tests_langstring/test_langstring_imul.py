...
def test_imul_with_various_multipliers(original_text: str, multiplier: int, expected_text: str) -> None:
    ...
    assert lang_string.text == expected_text, f"Expected text after in-place multiplication: '{expected_text}', got: '{lang_string.text}'"
...
def test_imul_with_large_multiplier(original_text: str, large_multiplier: int) -> None:
    ...
    assert len(lang_string.text) == len(original_text) * large_multiplier, "Length mismatch after large in-place multiplication"
...
def test_imul_with_unicode_and_special_characters(original_text: str, unicode_multiplier: int) -> None:
    ...
    assert lang_string.text == original_text * unicode_multiplier, "Incorrect in-place multiplication with Unicode/special chars"

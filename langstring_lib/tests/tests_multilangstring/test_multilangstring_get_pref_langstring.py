...
def test_get_pref_langstring_multiple_entries_allow() -> None:
    ...
    assert mls.get_pref_langstring() == ["Hello",
                                         "Hi"], "Expected both 'Hello' and 'Hi' for the preferred language 'en'"


def test_get_pref_langstring_with_overwrite() -> None:
    """Test behavior when the control is set to OVERWRITE and multiple LangStrings are added for the preferred language."""
    lang_str1 = LangString("Hello", "en")
    lang_str2 = LangString("Hi", "en")
...
def test_get_pref_langstring_with_block_error() -> None:
    """Test behavior when the control is set to BLOCK_ERROR and a duplicate LangString for the preferred language is added."""
    lang_str1 = LangString("Hello", "en")
    lang_str2 = LangString("Hi", "en")

...
def test_equality_with_identical_multilangstrings(sample_langstrings_eq) -> None:
    """Test equality of two MultiLangString objects with identical mls_dict and preferred_lang.
    ...
    assert mls1 == mls2, "MultiLangString objects with identical mls_dict and preferred_lang should be equal."


def test_inequality_with_different_langstrings(sample_langstrings_eq) -> None:
    """Test inequality of two MultiLangString objects with different mls_dict.

    :param sample_langstrings: Fixture providing sample LangString objects.
    :return: Asserts that MultiLangString objects with different mls_dict are not equal.
    """
    ...
    assert mls1 != mls2, "MultiLangString objects with different mls_dict should not be equal."
...
def test_equality_with_different_control_values(sample_langstrings_eq) -> None:
    """Test equality of two MultiLangString objects with different control values but identical mls_dict and \
    preferred_lang.

    :param sample_langstrings: Fixture providing sample LangString objects.
    :return: Asserts that MultiLangString objects with different control values but identical mls_dict and \
    ...
    assert mls1 == mls2, (
        "MultiLangString objects with different control values but identical mls_dict and preferred_lang should "
        "be equal."
    )
...
def test_equality_with_partial_overlap_in_langstrings(sample_langstrings_eq) -> None:
    """Test equality of two MultiLangString objects with partial overlap in mls_dict."""
    mls1 = MultiLangString(*sample_langstrings_eq[:2], preferred_lang="en")  # First two mls_dict
    mls2 = MultiLangString(*sample_langstrings_eq[1:], preferred_lang="en")  # Last two mls_dict
    assert mls1 != mls2, "MultiLangString objects with partial overlap in mls_dict should not be equal."


def test_equality_with_different_order_of_addition(sample_langstrings_eq) -> None:
    """Test equality of two MultiLangString objects with mls_dict added in different order."""
    mls1 = MultiLangString(*sample_langstrings_eq, preferred_lang="en")
    mls2 = MultiLangString(*reversed(sample_langstrings_eq), preferred_lang="en")
    assert mls1 == mls2, "MultiLangString objects with same mls_dict in different order should be equal."

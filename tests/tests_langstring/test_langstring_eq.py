...
@pytest.mark.parametrize("text1, lang1, text2, lang2, expected", [
...
def test_eq_with_various_strings(text1: str, lang1: str, text2: str, lang2: str, expected: bool):
    ...
    assert (lang_string1 == lang_string2) is expected, f"Equality check failed for LangString objects with " \
...
def test_eq_with_none():
    ...
    """
    lang_string = LangString("Hello", "en")
    assert (lang_string == None) is False, "LangString object should not be equal to None"
...
def test_neq_different_texts(langstring_en_hello, langstring_en_hi):
    ...
    assert langstring_en_hello != langstring_en_hi, "LangStrings with different texts but same language should be unequal"
...
def test_neq_different_languages(langstring_en_hello, langstring_es_hello):
    ...
    assert langstring_en_hello != langstring_es_hello, "LangStrings with same text but different languages should be unequal"
...
def test_neq_same_text_language(langstring_en_hello, langstring_en_hello_duplicate):
    ...
    assert not (
            langstring_en_hello != langstring_en_hello_duplicate), "LangStrings with same text and language should be equal"
...
def test_neq_with_none(langstring_en_hello):
    ...
    assert langstring_en_hello != None, "LangString should be unequal to None"

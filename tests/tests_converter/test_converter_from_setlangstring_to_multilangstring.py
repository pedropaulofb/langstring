...
def test_setlangstring_to_multilangstring_with_multiple_texts():
    """
    ...
    result = Converter.from_setlangstring_to_multilangstring(set_lang_string)
    expected = MultiLangString(mls_dict={"en": {"Hello", "Hi"}})
    assert result == expected, "setlangstring_to_multilangstring should return MultiLangString with all texts"


def test_setlangstring_to_multilangstring_with_single_text():
    """
    ...
    assert result == expected, "setlangstring_to_multilangstring should handle single text correctly"


@pytest.mark.parametrize("invalid_input", [123, 5.5, True, None, [], {}, "string", MultiLangString()])
def test_setlangstring_to_multilangstring_invalid_type(invalid_input):
    """
    ...
    with pytest.raises(TypeError, match="Invalid input argument type. Expected SetLangString, got"):
        Converter.from_setlangstring_to_multilangstring(invalid_input)

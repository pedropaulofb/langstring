...
def test_center_normal_cases(text, width, fillchar, expected):
    ...
    assert (
        centered.text == expected and centered.lang == lang_str.lang
    ), f"Expected centered text '{expected}', got '{centered.text}'"


@pytest.mark.parametrize("text, width", [("Hello", -1), ("Hello", 0)])
def test_center_invalid_width(text, width):
    """Test centering with invalid width values."""
    lang_str = LangString(text, "en")
    centered = lang_str.center(width)
    assert (
        centered.text == text and centered.lang == lang_str.lang
    ), f"Expected original text '{text}' for invalid width, got '{centered.text}'"


@pytest.mark.parametrize("width, fillchar, expected", [(10, " ", "          "), (5, "*", "*****"), (0, "-", "")])
...
@pytest.mark.parametrize("text, width, fillchar", [("Hello", 10, 123), ("Hello", 10, None), ("Hello", 10, True)])

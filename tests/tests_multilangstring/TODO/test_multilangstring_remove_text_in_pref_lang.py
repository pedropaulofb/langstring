...
@pytest.mark.parametrize("initial_contents, pref_lang, text_to_remove, clean_empty, expected_contents", [
...
def test_remove_text_in_pref_lang_valid_cases(initial_contents, pref_lang, text_to_remove, clean_empty, expected_contents):
    ...
    assert mls.mls_dict == expected_contents, "The MultiLangString contents after removal did not match the expected result."

import pytest
from langstring import MultiLangString

@pytest.mark.parametrize("initial_contents, pref_lang, text_to_remove, clean_empty, expected_exception, expected_message", [
    # Removing text not in preferred language
    ({"en": {"world"}, "fr": {"bonjour"}}, "en", "bonjour", False, ValueError, "Entry 'bonjour@en' not found in the MultiLangString."),
    ...
    ({"en": {" ", "world"}}, "en", " ", False, ValueError, "Text ' ' not found in preferred language 'en'"),
    # Text with emojis that does not exist
    ({"en": {"hello", "world"}}, "en", "goodbye 😊", False, ValueError, "Entry 'goodbye 😊@en' not found in the MultiLangString."),
    # Non-ASCII text not found
    ({"ru": {"привет", "мир"}}, "ru", "до свидания", False, ValueError, "Entry 'до свидания@ru' not found in the MultiLangString."),

])
def test_remove_text_in_pref_lang_invalid_cases(initial_contents, pref_lang, text_to_remove, clean_empty, expected_exception, expected_message):
